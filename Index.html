...
<body class="bg-slate-50 text-slate-900 pb-20 md:pb-0">
    <div id="app" class="flex flex-col md:flex-row h-screen overflow-hidden">
        ...
        <aside class="hidden md:flex flex-col w-72 bg-white border-r border-slate-200 p-6">
            ...
            <nav class="space-y-1 flex-1" id="main-nav">
                <button onclick="changeTab('dashboard')" class="nav-btn w-full flex items-center space-x-3 px-4 py-3 rounded-xl transition-all active-nav" data-tab="dashboard">
        ...
        <main class="flex-1 overflow-y-auto p-4 md:p-8" id="content-area">
            ...
            <div id="tab-report" class="tab-content hidden max-w-4xl mx-auto space-y-8">
                <div class="bg-white rounded-[2rem] shadow-xl border border-slate-100 p-8 space-y-8">
                    <h2 class="text-center text-3xl font-black">Laba Rugi</h2>
                    <div id="pnl-content">
                        <!-- JS Inject -->
                    </div>
                </div>
    ...
    <script>
        ...
        const appId = typeof __app_id !== 'undefined' ? __app_id : 'palm-finance-app';
        ...
        const startSync = () => {
            ...
            db.collection(`${userPath}/tbs_transactions`).onSnapshot(snap => {
            ...
            db.collection(`${userPath}/expenses`).onSnapshot(snap => {
        ...
        const updateUI = () => {
            const tonIn = allTransactions.filter(t => t.type === 'pembelian').reduce((a, b) => a + b.netWeight, 0);
            const tonOut = allTransactions.filter(t => t.type === 'penjualan').reduce((a, b) => a + b.netWeight, 0);
            const buyVal = allTransactions.filter(t => t.type === 'pembelian').reduce((a, b) => a + b.totalPrice, 0);
            const sellVal = allTransactions.filter(t => t.type === 'penjualan').reduce((a, b) => a + b.totalPrice, 0);
            const expVal = allExpenses.reduce((a, b) => a + b.amount, 0);
            ...
            document.getElementById('history-table-body').innerHTML = allTransactions.map(t => `
                <tr class="text-sm">
                    ...
                    <td class="px-6 py-4 text-center">
                        <button onclick="deleteDoc('tbs_transactions', '${t.id}')" class="text-slate-300 hover:text-rose-500"><i data-lucide="trash-2" size="16"></i></button>
                    </td>
                </tr>
            `).join('');
            ...
            document.getElementById('expense-table-body').innerHTML = allExpenses.map(e => `
                <tr class="hover:bg-slate-50">
                    ...
                    <td class="px-6 py-4 text-center">
                        <button onclick="deleteDoc('expenses', '${e.id}')" class="text-slate-300 hover:text-rose-500"><i data-lucide="trash-2" size="16"></i></button>
                    </td>
                </tr>
            `).join('');
            ...
            document.getElementById('pnl-content').innerHTML = `
                <div class="space-y-6">
                    ...
                    <div class="flex justify-between py-4 bg-slate-50 px-4 rounded-xl font-black"><span>LABA KOTOR</span><span>${formatIDR(sellVal - buyVal)}</span></div>
                    <div class="py-2">
                        <p class="text-xs font-black text-slate-400 uppercase mb-2">Beban Operasional</p>
                        ${allExpenses.map(e => `<div class="flex justify-between text-sm py-1"><span>${e.note}</span><span>${formatIDR(e.amount)}</span></div>`).join('')}
                    </div>
        ...
        const setTransType = (type) => {
            ...
            if (type === 'pembelian') {
                btnP.className = 'px-4 py-1.5 rounded-lg text-[10px] font-black uppercase transition-all bg-white text-emerald-600 shadow-sm';
                btnJ.className = 'px-4 py-1.5 rounded-lg text-[10px] font-black uppercase transition-all text-slate-400';
            } else {
                btnJ.className = 'px-4 py-1.5 rounded-lg text-[10px] font-black uppercase transition-all bg-white text-blue-600 shadow-sm';
                btnP.className = 'px-4 py-1.5 rounded-lg text-[10px] font-black uppercase transition-all text-slate-400';
            }
            recalculate();
        };
        ...
        const changeTab = (tabId) => {
            ...
            document.querySelectorAll('.nav-btn, .mobile-nav-btn').forEach(btn => {
                if (btn.dataset.tab === tabId) {
                    btn.classList.add('text-emerald-600', 'active-nav');
                    if(btn.classList.contains('nav-btn')) btn.classList.add('bg-emerald-600', 'text-white');
                } else {
                    btn.classList.remove('text-emerald-600', 'active-nav', 'bg-emerald-600', 'text-white');
